...
import { useUser } from "@/shared/components/app-shell";
...
export default function ChatThreadPage() {
  ...
  const meId = useUser()?.userId ?? null;
  ...
  // meId comes from UserContext (set synchronously before children render) but
  // WS handlers need the latest value without re-subscribing on changes.
  ...
  // Initial loads — detail and history — fired in parallel. meId comes from
  // AppShell's UserContext so no extra /api/auth/me call is needed here.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
